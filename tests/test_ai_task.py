...
async def test_generate_data_simple_text(
    ...
    if AI_TASK_AVAILABLE:
        ...
        chat_log = MagicMock()
        chat_log.content = [
            MagicMock(content="Generate a summary", role="user")
        ]
        chat_log.conversation_id = "test_conversation"
        
        # Mock the handler
        with patch.object(entity, '_async_handle_chat_log', return_value={"response": "This is a summary"}):
...
async def test_generate_data_with_structure(
    ...
    if AI_TASK_AVAILABLE:
        ...
        chat_log = MagicMock()
        chat_log.content = [
            MagicMock(content="Generate user data", role="user")
        ]
        chat_log.conversation_id = "test_conversation"
        
        # Mock the handler to return structured data
        structured_response = {"name": "John Doe", "age": 30}
        with patch.object(entity, '_async_handle_chat_log', return_value={"data": structured_response}):
            result = await entity._async_generate_data(task, chat_log)
